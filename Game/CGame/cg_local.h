...
// How long the deny mark stays lit on the icon a refused request was reaching
// for. Long enough to read as a reaction to the notch that caused it, short
// enough that a second notch answers with its own flash rather than extending
// the first.
#define	SKILL_REFUSE_TIME	500
...
typedef struct {
	...
	// the last skill change pmove turned down: when, which icon the player was
	// reaching for, and the reason it sent back
	int			skillRefuseTime;
	int			skillRefuseIndex;
	int			skillRefuseReason;
	...
	// The master this objective sends the player to, by name, or empty when the
	// objective is not a journey. The radar marks him as the destination.
	char		trainingDestination[MAX_QPATH];
...
typedef struct {
	...
	// The training treatment's support art, generated by
	// Tools/dev/make_training_art.py. trainingBarFillShader and its tip are
	// shader scripts rather than bare images because they are additive.
	qhandle_t	trainingCapShader;
	qhandle_t	trainingPlateShader;
	qhandle_t	trainingSashShader;
	qhandle_t	trainingSashDoneShader;
	qhandle_t	trainingBarTrackShader;
	qhandle_t	trainingBarFillShader;
	...
	// The training waypoints: every placed master, and the one the active
	// objective points at. Generated by Tools/dev/make_training_art.py.
	qhandle_t	RadarMasterShader;
	qhandle_t	RadarQuestShader;
	// The skill bar's refusal mark: dim over a skill whose requirements are not
	// met, lit in the threat colour over one a change was just refused for.
	qhandle_t	skillDenyShader;
	sfxHandle_t	skillDeniedSound;
...
// The skill bar's locked and refused marks. Archived like every other cg_draw*
// switch, so a player who wants the bar bare can keep it that way.
extern	vmCvar_t		cg_drawSkillState;
extern	vmCvar_t		cg_drawMasterMarks;
...
// cg_text.c
//
// Three baked faces. `display` carries the gold-to-white gradient the deck
// bakes down its glyphs, `displayw` is the same sheared cut in flat white for
// the sizes a gradient only muddies, and `body` is the compact upright the
// sentences, labels and status tags are set in - tinted at draw time, which is
// how one atlas produces jade DONE, blue OPEN and grey LOCK.
// Every panel edge in this UI is cut at 20 degrees and every display glyph
// leans 7. The two angles are the deck's and differ on purpose - a slab cut at
// the type angle reads as a mistake beside the type. The glyph shear is baked
// into the atlas, so only the slab angle is needed in code.
#define	TRAINING_SHEAR	0.364f
...
#define	TEXTF_SHADOW	1
#define	TEXTF_RIGHT		2
#define	TEXTF_CENTER	4
...
void		CG_DrawTrainingPic( float x, float y, float width, float height, const vec4_t color,
				qhandle_t shader );
