...
void SV_ExecuteClientMessage( client_t *cl, msg_t *msg ) {
	...
	// the subtraction is done on the sequence side so a client claiming
	// reliableAcknowledge == 0x7FFFFFFF cannot make the comparison wrap
	...
	if ( serverId != sv.serverId && !*cl->downloadName && !strstr(cl->lastClientCommandString, "nextdl") ) {
		...
		// only a client still connecting or priming can legitimately have
		// missed the gamestate; an active one asking for it is spoofing
