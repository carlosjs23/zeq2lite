...
usage:
    aura_reference_clean.py <in.png> <out.png> [--neutral 22] [--edge 6]
"""
...
def write_grey(path, g):
    h, w = g.shape
    raw = b""
    for y in range(h):
        raw += b"\x00" + g[y].tobytes()
...
def main():
    ...
    if A.max() - A.min() > 32:
        out = lum * (A / 255.0)
        write_grey(args.outfile, np.clip(out, 0, 255).astype(np.uint8))
        print("%s: real alpha, composited over black" % args.outfile)
        return

    corners = [lum[2, 2], lum[2, -3], lum[-3, 2], lum[-3, -3]]
    if max(corners) < 32:
        write_grey(args.outfile, np.clip(lum, 0, 255).astype(np.uint8))
        print("%s: already on black, luminance passed through" % args.outfile)
        return
    ...
    core = (chroma < args.neutral + 4) & (out > 40)
