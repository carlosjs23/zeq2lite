...
Training also gives the tier/`breakLimit` system somewhere to lead. On `master`
it is implemented and almost invisible — `cg_draw.c:700` draws one static icon
for what is really a progress bar — though `hud-stat-gauges` has since given it
its own gauge row and a top-up flash (`HUD_ROW_BL_Y`, `breakLimitReadyTime`,
`BREAKLIMIT_FLASH_TIME`).
...
2. Melee state and charge readout, borrowing the gauge vocabulary from
   `hud-stat-gauges` (`HUD_BAR_WIDTH`, `HUD_GAUGE_INSET`, the row heights).
3. Only then judge whether melee needs redesigning. It may already be the system
   we wanted.

**Do not put the charge meter in the status panel.** That panel is a fixed
288x86 plate with hardcoded rows (`HUD_ROW_PL_Y`, `HUD_ROW_HP_Y`, `HUD_ROW_ST_Y`,
`HUD_ROW_BL_Y`) whose sizes are coupled to art generated by
`Tools/dev/make_hud_gauge.py` — the header warns to "change them together or the
frames land off their windows". Adding a fifth row means regenerating the plate.

More importantly it is the wrong place: the panel is a status readout in the
corner, while melee charge and breaker windows are transient, sub-second, and
happen where the player is actually looking — at the locked target. Draw them
there. Same reasoning applies to the training objective tracker.
