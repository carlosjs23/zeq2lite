...
void main(void) {
	...
	/*
	 * Texture coordinates arrive finished: tr_shade.c resolves tcGen and every
	 * tcMod on the CPU and binds one array per texture unit, so each unit takes
	 * its own coordinate. Selecting between the two here - as this used to -
	 * would undo that, handing unit 1 the coordinates computed for unit 0.
	 */
